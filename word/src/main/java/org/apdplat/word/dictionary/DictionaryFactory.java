...
public final class DictionaryFactory {
    ...
    private static boolean usingKVFile=false;
    
    public static void setUsingKVFile(boolean usingKVFile) {
		DictionaryFactory.usingKVFile = usingKVFile;
	}
            ...
            AutoDetector.loadAndWatch(new ResourceLoader() {
				...
				private List<String> getAllWords(List<String> lines) {
					return lines.stream().flatMap(line -> getWords(line).stream())
							.filter(w -> w.length() <= INTERCEPT_LENGTH).collect(Collectors.toSet()).stream().sorted()
							.collect(Collectors.toList());
				}

				//企业ID:名称1,名称2,名称3,名称4
				private List<String> getAllWordsWithID(List<String> lines) {
					return lines.stream().filter(w -> w.length()>1).flatMap(line ->{
							String[] kv=line.split(":");//TODO #
							String nameList=kv[1];
							String nameValue=kv[0];
							//System.out.println(nameList);
							getWords(nameList).stream().forEach(item->KV.data.put(item, nameValue));
							return getWords(nameList).stream();
						})
							.filter(w -> w.length() <= INTERCEPT_LENGTH).collect(Collectors.toSet()).stream().sorted()
							.collect(Collectors.toList());
				}				
